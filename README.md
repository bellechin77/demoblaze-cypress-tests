...
| Given | When | Then |
| :---- | :---- | :---- |
| A user is on the home page of the application. The user wants to send a message via the contact form. The user clicks on the “Contact” link in the navigation bar. | The user enters a valid email address. The user enters his name. The user types a message in the text area. The user submits the contact form.  | The system displays an alert message confirming "Thanks for the message". The user remains on the home page.  |
...
| :---- | :---- |
| signup | Fill in username and password in the sign up form ``cy.get(`#sign-username`).type('testuser' + Date.now()) cy.get(`#sign-password`).type('testpassword')`` Click the ‘Sign up’ button `cy.get('button').contains('Sign up').click()` |
| contact | Fill in recipient email, recipient name and message text in the contact form ``cy.get(`#recipient-email`).type('abc@example.com’) cy.get(`#recipient-name`).type('Abc co.') cy.get(`#message-text`).type('This is a test contact message')`` Click the ‘Send message’ button `cy.get('button').contains('Send message').click()` |
| shoppingCart | Select a specific category from the product catalog `cy.get('[id="itemc"]').contains('Monitors').click()` Select a specific product from the catalog       `cy.get('.card-title').find('a').contains(productTitle).should('be.visible').click({force:true})` Click the ‘Add to cart’ button on the product page `cy.contains('Add to cart').click()` |
...
| :---- | :---- |
| Carousel.ts | The use of `data-slide` attribute is isolated from CSS style or JS behavioral changes. Select the next button in the carousel on the home page `cy.get('[data-slide="next"]')`  Select the previous button in the carousel on the home page `cy.get('[data-slide="prev"]')` |
| CartPage.ts | The use of `data-target` attribute is not coupled to the behavior or styling of an element. Select the ‘Place Order’ button on the cart page `cy.get('[data-target="#orderModal"]')` |

#### (3) Interactions with elements with a parent-child structure 

| Test | Interaction with parent-child structure |
| :---- | :---- |
| signup | Select the ‘Sign up’ link from the navigation menu `cy.get('.nav-link').contains(‘Sign up’)` |
| contact | Select the ‘Contact’ link from the navigation menu `cy.get('.nav-link').contains(‘Contact’)` |
| shoppingCart | Select a specific category from the product catalog `cy.get('[id="itemc"]').contains('Phones') cy.get('[id="itemc"]').contains('Laptops') cy.get('[id="itemc"]').contains('Monitors')` |

#### (4) Interactions with sibling elements 

| Test | Interaction with sibling elements in the product catalog |
| :---- | :---- |
| shoppingCart | Select the first product in the catalog `cy.get('.card-title').find('a').first()` |
| shoppingCart | Select the last product in the catalog `cy.get('.card-title').find('a').last()` |
| shoppingCart | Select a specific product in the catalog `cy.get('.card-title').find('a').contains(productTitle)` |
...
| :---- | :---- | :---- |
| signup | Check visibility of an element `.should('be.visible')` Check an element has a specific text `.should('have.text', 'Log in')` | Check the success alert message `cy.on('window:alert', (message) => { expect(message).to.contains(‘Sign up successful’)})` |
| contact | Check visibility of an element `.should('be.visible')` Check an element contains a specific URL ``.should('include', `${Cypress.config('baseUrl')}`)`` | Check the success alert message `cy.on('window:alert', (message) => { expect(message).to.contains(‘Thanks for the message’)})` |
| shoppingCart | Check visibility of an element `.should('be.visible')` Check the number of elements in the cart `.should('have.length', 3)` | Check the success alert message `cy.on('window:alert', (message) => { expect(message).to.contains(‘Product added’)})` |
...
| Debug Using `.log()` & `cy.pause()` | `cy.get('.submit-btn').log('Checking button').click() cy.pause()` | This helps you inspect the test state before proceeding |
...
| Validate API Responses in Network Tab  | `cy.intercept('GET', '/api/cart').as('getCart') cy.wait('@getCart').its('response.statusCode').should('eq', 200);`  | This ensures the API responded correctly before continuing |
